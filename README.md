# Safety Gateway Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/safety_gateway.svg?label=pypi%20(stable))](https://pypi.org/project/safety_gateway/)

The Safety Gateway Python library provides convenient access to the Safety Gateway REST API from any Python 3.9+
...
pip install git+ssh://git@github.com/xaviour098/PGuard.git
```
...
```sh
# install from the production repo
pip install 'safety_gateway[aiohttp] @ git+ssh://git@github.com/xaviour098/PGuard.git'
```
...
```python
import asyncio
from safety_gateway import DefaultAioHttpClient
from safety_gateway import AsyncSafetyGateway
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `safety_gateway.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `safety_gateway.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `safety_gateway.APIError`.

```python
import safety_gateway
from safety_gateway import SafetyGateway
...
except safety_gateway.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except safety_gateway.APIStatusError as e:
...
You can enable logging by setting the environment variable `SAFETY_GATEWAY_LOG` to `info`.

```shell
$ export SAFETY_GATEWAY_LOG=info
```
...
```python
import httpx
from safety_gateway import SafetyGateway, DefaultHttpxClient

client = SafetyGateway(
    # Or use the `SAFETY_GATEWAY_BASE_URL` env var
...
```py
import safety_gateway
print(safety_gateway.__version__)
```
